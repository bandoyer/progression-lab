...
To update the database use this command from the Web project folder (replace `ProgressionLab` with your project's name):

```powershell
dotnet ef database update -c AppDbContext -p ../ProgressionLab.Infrastructure/ProgressionLab.Infrastructure.csproj -s ProgressionLab.Web.csproj
```
